...
「速构智维」是本项目的主插件（`速构智维.pyplugin`，v1.2.0，SDK 1.6.2），一个内嵌于 BIMBase 的 **PyQt 简化版 CAD 画板**，右侧集成 **AI 助手面板**，外加 **自动配置环境** 与 **桥梁病害识别（转发入口）** 两个面板按钮。
...
| 桥梁病害识别 | `bridge_disease_launcher.py` | 仅转发：把 `../桥梁病害识别` 加入 sys.path 后调用该模块入口（功能已拆分，见 `桥梁病害识别/README.md`） |
...
pyp3d 放置必须在窗口事件循环的交互回调链中执行，且 速构智维 与 AI_Modeling 曾有同名 `bimbase_sync.py` 导致组件类解析错乱、BIMBase 崩溃。方案（`ai_modeling_launcher.py:109-209`）：
...
- **识别三视图构件的同步定位**（2026-09 修复链）：同步弹窗输入的基准坐标是唯一放置依据——识别构件带 `_recognized_component` 标记，「全部同步」时只同步构件、不同步图纸线条；门式桥墩等 4 条生成代码路径一律优先用基准坐标；重复同步会删除旧实例**及其绑定的图形实体**（避免旧组件残留成"幽灵"被误选）。
- **AI 指令同步坐标**：「把画板组件同步到一万一万一万的位置」这类语音指令支持中文数字解析（`ai_panel.py::_extract_position` 兜底），解析失败弹手动输入框，不再静默同步到错误位置。

### 4.5 命令系统修复（2026-09）

- **命令行回车激活命令**：输入 `TR`/`延伸` 等命令名或快捷键回车即可激活对应命令（`_on_cmd_return` 新增命令名解析分支，此前只能输坐标/参数）；
- **8 个修改命令修复**：延伸（重写为单步点选）、拉伸（`_sel_box` 判断恒真 bug）、偏移（默认距离调 10mm）、倒斜角/圆角（重写为点选两线+命令行输参数，去掉内嵌环境不可用的模态弹窗）、合并/分解/等分（原逻辑验证可用）。离线桩件回归 12/12 通过（`archive/test_modify_commands.py`）。
...
| 文件 | 怎么跑 | 测什么 |
|---|---|---|
| `test_ai_modeling_bridge.py` | `python 速构智维/test_ai_modeling_bridge.py` | AI 路由单元测试（纯标准库，无需 BIMBase/Qt） |
| `test_cached_component_ops.py` | `python 速构智维/test_cached_component_ops.py` | 缓存组件 AI 操作回归（无需 BIMBase） |
| `test_audio.py` | `python 速构智维/test_audio.py` | 音频设备诊断（需 PyQt5） |
...
> 注意：根目录下的 `dwg_handler.py`、`modify_edit.py`、`modify_advanced.py`、`snap.py` 是旧版遗留副本，实际生效的是 `utils/`、`commands/` 下的新版；`board_patch.py` 等 patch 脚本为开发期一次性工具；`BIMPyPlugin.xml` 是旧清单副本，以根目录 `速构智维.pyplugin` 为准。

## 文档

- [CAD画板功能说明](docs/CAD画板功能说明.md)
- [AI智能建模说明](docs/AI智能建模说明.md)
- [桥梁病害识别说明](docs/桥梁病害识别说明.md)
