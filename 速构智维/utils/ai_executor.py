...
class AICommandExecutor:
    ...
    def _do_sync(self, cmd):
        ...
        # 语音/自然语言坐标解析失败时，弹出基准坐标对话框让用户手动输入
        # （与画板"同步到BIMBase"按钮的交互一致，避免静默同步到错误位置）
        if not position:
            try:
                origin = self.board._ask_sync_origin()
            except Exception:
                origin = None
            if origin is None:
                return False, "已取消同步（未提供基准坐标）"
            position = {'mode': 'manual', 'x': origin[0], 'y': origin[1], 'z': origin[2]}
        target = cmd.get('target', {})
        elems = []
        if target:
            elems = self._resolve_target(target)
