...
class LocalCommandParser:
    ...
    def _extract_position(cls, text: str):
        ...
        for pat in patterns:
            ...
            if m:
                ...
                return pos
        # 中文数字坐标（语音输入常见）：如"同步到一万一万一万"→(10000,10000,10000)
        # 按"万"切分组逐组转换；组数不为 2/3 则交给上层弹窗兜底
        m = re.search(r'(?:同步到|放在|坐标|位置|到)\s*(?:bimbase\s*的?\s*)?([零一二两三四五六七八九十百千万\s]+)', text)
        if m:
            seg = m.group(1).strip()
            groups = [g + '万' for g in seg.split('万') if g] if '万' in seg else [seg]
            vals = [cls._cn_to_num(g) for g in groups]
            vals = [v for v in vals if v is not None]
            if 2 <= len(vals) <= 3:
                pos = {'x': float(vals[0]), 'y': float(vals[1])}
                if len(vals) == 3:
                    pos['z'] = float(vals[2])
                return pos
        return None
    ...
    @staticmethod
    def _cn_to_num(s):
        """中文数字转数值（简易版）：一万→10000，三百→300，二十→20"""
        ...
        for ch in s:
            if ch in digits:
                ...
                u = units[ch]
                if u == 10000:
                    total = (total + num) * u if total else num * u
                    num = 0
                else:
