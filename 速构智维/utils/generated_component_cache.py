...
CADBOARD_DIR = os.path.join(PROJECT_ROOT, '速构智维')
...
def clear_cache_dir():
    """清除 cad组件缓存 目录下的缓存（不删 .py 生成脚本——它们是已放置组件的类定义，
    BIMBase 反序列化参数时需要；只清 __pycache__ 强制重新编译）"""
    if not os.path.isdir(CACHE_DIR):
        return 0
    removed = 0
    ...
    _log(f"cleared {removed} cache items (.py scripts kept)")
    return removed
