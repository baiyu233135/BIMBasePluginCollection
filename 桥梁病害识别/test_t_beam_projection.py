...
使用方法（在 BIMBase Python 控制台或 exec 环境中运行）：
    exec(open('桥梁病害识别/test_t_beam_projection.py', encoding='utf-8').read())

说明：
- 使用 速构智维/bimbase_sync.py 的 _PlaceToDirect 实现自动放置，不进入手动布置模式。
...
def place_component_direct(comp, x, y, z):
    """
    使用 速构智维/bimbase_sync.py 的 _PlaceToDirect 自动放置组件。
    不进入手动布置模式。
    """
    try:
        _module_dir = os.path.dirname(os.path.abspath(__file__))
        _cadboard_dir = os.path.normpath(os.path.join(_module_dir, '..', '速构智维'))
