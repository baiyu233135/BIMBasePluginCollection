# -*- coding: utf-8 -*-
"""
隧道病害检测模型训练脚本 — YOLOv8n

基于人工标注数据集训练轻量级病害检测模型，支持CPU训练（较慢）。
...
使用方法:
    # 准备数据集后（数据集由 training/local_annotator.py 导出 data.yaml）
...
def main():
    parser = argparse.ArgumentParser(description="训练隧道病害检测YOLOv8n模型")
    parser.add_argument("--data", "-d", default="../datasets/tunnel_disease/data.yaml",
                       help="数据集YAML配置文件路径 (默认: ../datasets/tunnel_disease/data.yaml)")
    ...
    if not os.path.exists(args.data):
        ...
        print("请先用 training/local_annotator.py 标注并导出数据集")
        sys.exit(1)
