# -*- coding: utf-8 -*-
"""
隧道病害标记模块 — 在BIMBase隧道构件表面生成彩色标记

通过创建独立的彩色标记组件并放置到BIMBase中对应构件位置，
...
# 点云标记点数硬上限（防呆）：每个点最终变成一个 pyp3d 小立方体对象，
# 超过该值会导致 BIMBase 卡死（未响应）。正常路径由
# face_projection.PointCloudGenerator 的自适应上限控制到此值以下。
POINT_CLOUD_HARD_LIMIT = 3000
...
if _pyp3d_ok and Component is not None:
    ...
    class _DiseasePointCloudMarker(Component):
        ...
        参数:
            x, y, z    : 点云基准位置（面中心）
            points     : 点列表，每个点为 (local_u, local_v, depth, (r, g, b))
                         depth 含义随 face_plane：平面面为法向偏移量；
                         face_plane='xyz' 时整组 (u,v,depth) 即真 3D 局部坐标
                         （弧面点云，每点深度不同，无法再用平面映射表达）
            point_size : 每个小点的尺寸(mm)
            face_plane : 面类型 'xz'/'xy'/'yz'/'xyz'
        """
        def __init__(self, x=0, y=0, z=0, points=None, point_size=3.0, face_plane='xz',
                     disease_type=''):
            super().__init__()
            points = list(points or [])
            if len(points) > POINT_CLOUD_HARD_LIMIT:
                _log(f"点云点数 {len(points)} 超过硬上限 "
                     f"{POINT_CLOUD_HARD_LIMIT}，已截断（防止BIMBase卡死）")
                points = points[:POINT_CLOUD_HARD_LIMIT]
            self['x'] = Attr(float(x), show=True, obvious=True)
            self['y'] = Attr(float(y), show=True, obvious=True)
            self['z'] = Attr(float(z), show=True, obvious=True)
            ...
            try:
                ...
                for u, v, z, (r, g, b) in points:
                    ...
                    if plane == 'xyz':
                        ...
                        px, py, pz = u, z, v
                    sx, sy, sz = ps, ps, ps
                    small = translate(px, py, pz) * scale(sx, sy, sz) * Cube().color(cr, cg, cb, 0.85)
                    parts.append(small)
...
class DiseaseMarkerComponent:
    ...
    def create_point_cloud_marker(x: float, y: float, z: float,
        ...
        Args:
            ...
            points: 点列表，每个点为 (local_u, local_v, depth, (r, g, b))；
                    face_plane='xyz' 时 (u,v,depth) 为真 3D 局部坐标（弧面点云）
            point_size: 每个小点的尺寸(mm)
            face_plane: 面类型 'xz'/'xy'/'yz'/'xyz'
    ...
    def place_marker(self, photo_path: str, component_type: str, 
        ...
        if _pyp3d_ok and place is not None:
            try:
                ...
                if comp is not None:
                    # 隧道版：bimbase_sync 已复制到本插件目录，不再依赖 速构智维
                    _plugin_dir = os.path.dirname(os.path.abspath(__file__))
                    if _plugin_dir not in sys.path:
                        sys.path.insert(0, _plugin_dir)
                    import bimbase_sync as _ref
                    original_argv0 = sys.argv[0]
                    sys.argv[0] = os.path.join(_plugin_dir, 'bimbase_sync.py')
                    try:
                        place(comp)
                    finally:
