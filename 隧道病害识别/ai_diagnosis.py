...
def get_config_path() -> str:
    """获取配置文件路径"""
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "tunnel_disease_config.json")
...
def _log(msg: str):
    """写调试日志"""
    try:
        log_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tunnel_disease_debug.log")
        from datetime import datetime
        with open(log_path, "a", encoding="utf-8") as f:
            f.write(f"[{datetime.now().strftime('%H:%M:%S')}] [TUNNEL_AI_DIAG] {msg}\n")
    except Exception:
        pass
...
def _build_prompt(bboxes: List[Tuple[int, int, int, int]], component_type: str) -> str:
    ...
    prompt = f"""你是一位资深的隧道病害检测专家（依据《公路隧道养护技术规范》JTG H12-2015）。我将提供两张图片：

第一张是原始照片，第二张是用红色矩形框标出了异常区域的同一张照片。
这些照片拍摄的是隧道的 {component_type} 构件表面。
...
对每个区域给出以下信息：
1. 病害类型：从 [裂缝、渗水、剥落、其他] 中选择，若无法判断则填"其他"。
2. 严重程度：轻微 / 中等 / 严重 / 极严重（对应隧道状况值 1~4）。
3. 位置：用一句话描述该异常在照片中的相对位置（如"衬砌拱顶右侧"）。
4. 尺寸：用像素或估算实际尺寸描述。
5. 诊断建议：给出专业的处理或复查建议。
...
{{
  "diagnoses": [
    {{
      ...
      "position": "衬砌边墙中部偏上",
      "size": "约 150×100 像素",
      "diagnosis": "衬砌表面渗漏水伴钙质结晶，建议查明水源并采取注浆堵漏措施。"
    }},
    ...
  ]
}}
...
def diagnose_image(
    ...
    if not api_key:
        raise ValueError("未配置 dashscope_api_key，请先填写 隧道病害识别/tunnel_disease_config.json")
    ...
    try:
        ...
        for i, diag in enumerate(diagnoses):
            ...
            diag.setdefault("disease_type", "其他")
