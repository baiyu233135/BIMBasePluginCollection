...
# 隧道版说明：本文件从 速构智维 原样复制，隧道病害识别为独立插件、不带 utils 子包，
# 以下三处导入改为容错模式——缺失时仅画板同步类（BIMBaseSync）内部功能降级，
# 病害识别管线依赖的实体查询 API（get_all_instancekey 等）不受影响。
try:
...
try:
    ...
    def apply_component_params_to_element(*args, **kwargs):
        return None

try:
    from utils.generated_component_cache import generate_pier_code, execute_generated_code
except Exception:
    generate_pier_code = None
    execute_generated_code = None
