...
处理建议措辞依据《公路隧道养护技术规范》JTG H12-2015 的常规养护处置要求。
"""
...
# 日志文件路径（与tunnel_disease_debug.log共用，便于排查）
...
def _log(msg: str):
    ...
    try:
        ...
        with open(_LOG_FILE, "a", encoding="utf-8") as f:
            f.write(f"[{ts}] [TUNNEL_REPORTGEN] {msg}\n")
    except Exception:
        pass


# 病害处理建议字典（隧道版，依据 JTG H12-2015 公路隧道养护技术规范）
# 严重程度四级：轻微 / 中等 / 严重 / 极严重（对应状况值 1~4）
DISEASE_TREATMENT_ADVICE = {
    "裂缝": {
        "轻微": "衬砌裂缝宽度小于 0.2mm，建议采用表面封闭法处理，涂刷环氧树脂或聚氨酯类密封材料，并列入日常巡查观察。",
        "中等": "裂缝有明显发展，建议采用压力灌浆法处理，注入环氧树脂浆液或水泥浆液，灌浆后封闭表面并定期复测。",
        "严重": "裂缝宽度较大或伴有错台、渗水，建议查明开裂原因，视情况采用嵌补加固、粘贴碳纤维布或钢带加固，并缩短检测周期。",
        "极严重": "裂缝贯通、交叉成网或衬砌明显变形，结构安全受威胁，建议立即限制交通，进行专项检测与结构验算，及时采取加固处治措施。",
    },
    "渗水": {
        "轻微": "衬砌表面潮湿或轻微渗水，建议查明水源，采用防水砂浆抹面或涂刷防水涂料处理，并观察发展情况。",
        "中等": "渗水成流或伴钙质析出，建议设置引排设施疏导，并对渗水部位进行化学注浆堵漏，冬季应防止冻胀。",
        "严重": "渗水量大或水压较高，建议全面排查地下水补给通道，采用帷幕注浆或径向注浆止水，必要时增设排水系统。",
        "极严重": "涌水夹泥、衬砌背后出现空洞，可能伴随围岩失稳，建议立即处治，先排后堵、排堵结合，并评估衬砌结构安全性。",
    },
    "剥落": {
        "轻微": "衬砌表层轻微起层剥落，建议清除松动部分，采用聚合物修补砂浆或喷射混凝土修补。",
        "中等": "剥落露骨料或局部露筋，建议凿除损坏区域至坚实基层，除锈后重新浇筑修补混凝土，恢复保护层厚度。",
        "严重": "剥落面积较大或呈块状掉块趋势，建议全面检测衬砌厚度及背后空洞，采用挂网喷射混凝土或镶补套拱加固。",
        "极严重": "衬砌掉块危及行车安全，建议立即清除危石并设置防护，尽快安排专项处治，必要时对衬砌进行拆换加固。",
    },
    "其他": {
        "轻微": "图像存在轻微异常（污渍、霉斑、轻微变形等），建议现场复核确认病害性质，列入日常养护记录。",
        "中等": "图像存在明显异常，建议进一步检测（敲击、测量、渗水试验）以确定病害类型和程度，并拍照存档。",
        "严重": "异常特征显著，建议尽快安排专业检测，明确病害成因与影响范围，制定相应处治方案。",
        "极严重": "异常可能危及结构安全或行车安全，建议立即组织专项检测与评估，必要时采取交通管制和应急处治措施。",
    },
}
...
class ReportGenerator:
    ...
    def __init__(self):
        self.project_name = "隧道病害检测项目"
        self.tunnel_name = "待填写"
        self.inspector = "AI自动检测系统"
        self.inspection_date = datetime.now().strftime("%Y年%m月%d日")

    def set_project_info(self, project_name: str = "", tunnel_name: str = "",
                         inspector: str = ""):
        """设置项目信息"""
        if project_name:
    ...
    def _add_project_overview(self, doc, records):
        ...
        doc.add_paragraph(f"本次检测共发现 {total_diseases} 处病害，涉及 {len(component_types)} 种隧道构件类型（衬砌/路面/洞门/检修道/排水）。")
        doc.add_paragraph("检测范围：左右洞衬砌、路面、检修道及排水设施，各病害桩号区间详见病害详图。")
        doc.add_paragraph()
...
def generate_report(records: list, output_path: str,
                    project_name: str = "", tunnel_name: str = "") -> bool:
    """便捷函数：一键生成报告"""
    gen = ReportGenerator()
    gen.set_project_info(project_name, tunnel_name)
