# -*- coding: utf-8 -*-
"""生成 20 页国创赛产业赛道 PPT v2：智检桥安（补充桥梁数字孪生在线平台）"""
import os, glob, sys
...
OUT = os.path.join(ROOT, "docs", "国创赛ppt", "国创赛产业赛道_智检桥安_v2.pptx")
...
layers = [
    ("应用层", "桌面检测插件 · 三维可视化 · Word 报告自动生成 · 网页端数字孪生平台", ORANGE),
...
for i, (t, d, col) in enumerate(layers):
    ...
    add_text(s, Inches(3.1), y + Inches(0.2), Inches(6.5), Inches(0.8),
             [(d, 12.5, DARK, False)])
# 层间箭头提示（用文字）
add_text(s, Inches(0.55), Inches(6.42), Inches(9.2), Inches(0.3),
         [("自下而上：数据 → 算法 → 引擎 → 应用（图中自下而上堆叠）", 10, DGRAY, False)],
         align=PP_ALIGN.CENTER)
bar7 = add_rect(s, Inches(0.55), Inches(6.72), Inches(12.25), Inches(0.42), fill=LBLUE,
                line=ORANGE, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
set_text(bar7.text_frame,
         [("桌面插件 + 在线平台双形态交付，浏览器免安装即可访问", 12, DARK, True)],
         align=PP_ALIGN.CENTER)
bar7.text_frame.paragraphs[0].alignment = PP_ALIGN.CENTER
ix = Inches(10.15)
...
cards = [
    ...
    ("桥梁数字孪生在线平台", "浏览器直接访问：三维模型浏览、损伤分布查看、检测报告在线浏览、多期检测数据对比与劣化趋势追踪"),
]
for i, (t, d) in enumerate(cards):
    if i < 3:
        x = Inches(0.6) + i * Inches(4.25)
        y, w, h = Inches(1.45), Inches(4.0), Inches(1.65)
        tx, tw = x + Inches(0.35), Inches(3.4)
    else:
        x = Inches(0.6) + (i - 3) * Inches(6.3)
        y, w, h = Inches(3.3), Inches(5.9), Inches(1.65)
        tx, tw = x + Inches(0.35), Inches(5.2)
    add_rect(s, x, y, w, h, fill=GRAY, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    add_rect(s, x, y, Inches(0.12), h, fill=BLUE)
    dsz = 12 if i < 3 else 11.5
    add_text(s, tx, y + Inches(0.18), tw, h - Inches(0.35),
             [(f"{i+1}. {t}", 15.5, DARK, True), (d, dsz, BODY, False)], space_after=6)
add_text(s, Inches(0.6), Inches(5.12), Inches(12.1), Inches(0.4),
         [("数字孪生在线平台：建设中，将提供可公开访问的演示网址", 12.5, ORANGE, True)], align=PP_ALIGN.CENTER)
pw = 3.9
placeholder(s, Inches(0.6), Inches(5.6), Inches(pw), Inches(1.35),
            "检测插件运行界面截图（运行后替换）", 12)
placeholder(s, Inches(4.7), Inches(5.6), Inches(pw), Inches(1.35),
            "三维可视化界面截图（运行后替换）", 12)
placeholder(s, Inches(8.8), Inches(5.6), Inches(pw), Inches(1.35),
            "数字孪生平台网页界面（上线后替换，附访问网址与二维码）", 12)
...
biz = [
    ...
    ("平台订阅", "数字孪生在线平台账号订阅、病害档案库托管、多期数据对比与劣化趋势分析"),
]
...
add_text(s, Inches(0.6), Inches(5.1), Inches(12.1), Inches(0.35),
         [("推广路径：在线平台免安装、扫码即开，降低试用与推广门槛", 12, DGRAY, True)], align=PP_ALIGN.CENTER)
