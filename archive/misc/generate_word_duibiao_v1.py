# -*- coding: utf-8 -*-
"""
生成《智检桥安-命题解决方案（对标版）v1.docx》—— 对标沈阳建筑大学模板格式。

项目：智检桥安——基于人工智能与计算机视觉的桥梁表观损伤智能检测系统
命题：基于人工智能与计算机视觉的桥梁表观损伤智能检测系统开发（命题企业：广联达）
输出：docs/国创赛ppt/智检桥安-命题解决方案（对标版）v1.docx
...
OUT_PATH = os.path.join(PPT_DIR, "智检桥安-命题解决方案（对标版）v1.docx")
...
def build_ch6(doc):
    ...
    add_h2(doc, "6.3  效果评价指标")
    add_body(doc, "案例验证的效果评价指标如表 6-1 所示。已完成的指标如实填写；预期口径的指标统一在表中标注"
                  "“预期”，测试完成后以实测数据如实回填。全部评价数据真实、清晰、可复核，测试脚本与原始记录"
                  "可备查。")
    add_tbl(doc, "表 6-1  案例验证效果评价指标",
            ["评价指标", "结果"],
            [["损伤识别准确率与召回率",
...
def main():
    ...
    doc = Document()
    setup_page(doc)
    doc.core_properties.title = "智检桥安-命题解决方案（对标版）v1"
    ...
    chapters = ["项目摘要", "一、项目概述与设计目标", "二、应用场景", "三、系统结构设计",
                "四、操作说明", "五、核心技术及原理", "六、项目实现与案例验证", "七、运行环境",
                "八、市场与竞争性分析", "九、产教协同与团队协作", "十、风险分析",
                "附录一", "附录二", "附录三", "附录四"]
