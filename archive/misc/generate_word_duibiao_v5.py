# -*- coding: utf-8 -*-
"""
生成《智检桥安-命题解决方案（对标版）v5.docx》—— 对标沈阳建筑大学模板格式。

项目：智检桥安——基于人工智能与计算机视觉的桥梁表观损伤智能检测系统
命题：基于人工智能与计算机视觉的桥梁表观损伤智能检测系统开发（命题企业：广联达）
输出：docs/国创赛ppt/智检桥安-命题解决方案（对标版）v5.docx

v5 在 v4 基础上修订：
  ① v2 模型实测数据就位（142 轮早停、最佳约 103 轮：mAP50=0.351、mAP50-95=0.158、
    P=0.414、R=0.413；v1 基线 0.336/0.085 保留作对比）；5.1 改写为"v1 基线＋v2 实测"
    双模型叙述，图 5-1 换用 模型性能基线与提升_v2.png；摘要、6.4 表、10.1、9.6、
    附录四等引用处同步；量化/定位/耗时仍为预期口径不动。
  ② 9.4 重写为四段结构（调研过程含【待补充】占位与真实发现、成长主线、攻关实录、
    人才培养与产教融合）。
  ③ 摘要末尾加获奖表述一句；附录一新增获奖证书扫描件占位条目。
...
OUT_PATH = os.path.join(PPT_DIR, "智检桥安-命题解决方案（对标版）v5.docx")
...
def build_abstract(doc):
    ...
    add_body(doc, "针对命题企业“基于人工智能与计算机视觉的桥梁表观损伤智能检测系统开发”的真实业务需求，"
                  ...
                  "采用YOLOv8n专用模型加免模型五路CV融合的双模式，模型在普通CPU电脑上完成训练：v1基线实测"
                  "mAP50约0.336、mAP50-95约0.085，v2改进模型实测mAP50=0.351、mAP50-95=0.158（142轮早停）——"
                  "指标还不算高，我们全部如实写出，并给出后续提升路线；诊断上集成Qwen-VL多模态大模型分析病害"
                  "成因；定位上用解析式2D→3D面投影把病害精确挂到BIM构件表面；最后用python-docx一键生成规范的"
                  "Word检测报告。")
    ...
    add_body(doc, "本项目相关技术体系曾获“挑战杯”全国大学生课外学术科技作品竞赛国家级奖励、“龙图杯”"
                  "全国BIM大赛国家级奖励等多项国家级奖项（证书见附录一）。")
    ...
    add_tbl(doc, "表 5-1  数据集建设情况",
            ...
            [["GYU-DET（公开数据集）", "11,123张", "已有标注（格式转换）", "训练/验证"],
             ...
             ["本地巡检报告提取与人工标注", "1,014张（整合集）", "YOLO格式人工标注",
              "贴近真实业务场景的训练/验证"]])
    add_body(doc, "（4）基线与 v2 实测性能：v1 基线模型在 CPU 环境下完成 50 个 epochs 训练，实测 mAP50 约"
                  "0.336、mAP50-95 约 0.085；v2 改进模型（YOLOv8n，计划 200 epochs、patience 30、batch 4、"
                  "imgsz 640，CPU）于第 142 轮早停、最佳约为第 103 轮，实测 mAP50=0.351、mAP50-95=0.158、"
                  "精确率 P=0.414、召回率 R=0.413。两轮训练基于同一数据集，results.csv 与训练曲线完整保存、"
                  "可复核。对比如图 5-1 所示：mAP50 由 0.336 提升至 0.351（+4.5%），mAP50-95 由 0.085 提升至 "
                  "0.158（+86%），高 IoU 阈值下的精细定位能力显著增强。上述指标均为真实训练实测，本说明书如实"
                  "呈现、不作夸大；")
    add_fig(doc, ppt_img("模型性能基线与提升_v2.png"), "图 5-1  模型性能基线与 v2 实测对比", width_cm=14.0)
    add_body(doc, "（5）提升路线：在 v2 实测基础上，后续将继续从数据增强（旋转、翻转、光照扰动、马赛克"
                  "拼接）、更大骨干网络（YOLOv8s/m 迁移学习）、半监督学习（未标注影像伪标签迭代）、补充标注与"
                  "类别均衡采样、小目标特征层优化五条路径持续提升；每轮改进均以同一验证集实测，指标如实更新，"
                  "原始记录可备查。")
...
def build_ch6(doc):
    ...
    add_tbl(doc, "表 6-2  案例验证效果评价指标",
            ["评价指标", "结果"],
            [["损伤识别准确率与召回率",
              "实测：v2 模型 mAP50=0.351、mAP50-95=0.158、P=0.414、R=0.413（142 轮早停，results.csv 可复核，"
              "见 5.1 节）；v1 基线 mAP50 约 0.336、mAP50-95 约 0.085 作为对比保留"],
             ["单张图像平均检测时间",
...
def build_ch9(doc):
    ...
    add_body(doc, "在方案设计前，我们围绕桥梁检测业务流程开展了需求调研：访谈检测单位工程师【待补充】次、"
                  "回收调研问卷【待补充】份（问卷与访谈提纲见附录五附 5-1、附 5-2，调研纪要按附 5-3 模板记录）。"
                  "调研中一个印象最深的发现是：病害照片与构件的对应关系至今仍靠人工记录——一张照片拍的是哪根墩、"
                  "哪个面，全凭外业人员的手写备注。正是这个发现，决定了本方案“病害必须定位到构件”的核心设计。"
                  "【待补充】调研纪要与试用反馈原件待整理扫描后归入附录五，本说明书不作虚构陈述。")
    add_body(doc, "作为团队负责人，回看整个过程，最深的体会是“课堂知识真的能变成工程能力”：《桥梁工程》讲的"
                  "构件体系，变成了我们划分检测对象和设计病害定位构件路由表的依据；《数字图像处理》讲的形态学"
                  "与边缘检测，变成了免模型五路 CV 掩码融合的五条通道；《机器学习》讲的目标检测原理与评估指标，"
                  "变成了基线模型 50 轮 CPU 训练的实验设计——而 mAP50 从 0.336 到 0.351 的每一步提升，背后都对应"
                  "着真实的数据清洗与调参记录，没有一步是写出来的。")
    add_body(doc, "攻关过程全程留痕：git 提交记录 60+ 次，对话记录与报错日志完整归档。印象最深的一次是病害"
                  "投影的“闪面”问题：投影贴片与构件表面共面，渲染时两者交替闪烁、无法观看。我们最终放弃了提高"
                  "渲染优先级的思路，改用解析式方案——让贴片沿构件面法向偏移 2mm，从几何上彻底消除共面，问题"
                  "一次性解决。这类过程材料（对话记录、报错日志、git 历史）均可备查。")
    add_body(doc, "项目依托【学院/实验室名称 待填写】开展，由指导教师【待填写】在桥梁工程与计算机视觉方向"
                  "把关指导；命题企业广联达提供真实业务场景与命题要求。沿着“企业出题、高校协同、学生攻关”的"
                  "路径，我们把课堂、实验室和真实工程连在了一起。")
    ...
    add_body(doc, "（2）工匠精神：指标如实写、不夸大——基线模型 mAP50 约 0.336、改进模型 0.351，都按实测"
                  "写明，预期指标逐处标明口径，测试数据与原始记录全程留痕、经得起查验；")
...
def build_ch10(doc):
    ...
    add_body(doc, "风险分析：当前 v2 模型实测 mAP50=0.351、mAP50-95=0.158（v1 基线为 0.336/0.085），距工程"
                  "实用水平尚有差距；微裂缝等小目标损伤识别难度大；复杂光照、遮挡与表面污渍可能引起误检漏检；"
                  "尺寸量化与定位精度尚待实测验证。")
...
def build_appendices(doc):
    ...
    add_body(doc, "（4）企业合作证明：【待补充】企业试用证明与合作说明材料待对接后补充，本说明书不作虚构"
                  "陈述；")
    add_body(doc, "（5）获奖证书：本项目相关技术体系曾获“挑战杯”全国大学生课外学术科技作品竞赛国家级奖励、"
                  "“龙图杯”全国 BIM 大赛国家级奖励等多项国家级奖项；获奖证书扫描件【占位：扫描后替换】"
                  "（挑战杯/龙图杯/新基建杯/数学建模，按证书原件如实放入）；")
    add_body(doc, "（6）数据集与第三方组件：GYU-DET、CODEBRIM 公开数据集按其各自许可协议使用；BIMBase、"
    ...
    add_body(doc, "（3）数据合规与脱敏：项目在数据采集与使用过程中严格遵守相关法律法规；涉及实际工程图像、"
                  ...
                  "中不出现未授权的工程敏感信息；")
    add_body(doc, "（4）真实性承诺：本说明书全部技术指标如实呈现——v1 基线模型 mAP50 约 0.336、mAP50-95 约 "
                  "0.085 与 v2 改进模型 mAP50=0.351、mAP50-95=0.158 均为真实训练实测结果（results.csv 可复核）；"
                  "尺寸量化精度、定位精度与检测耗时为预期口径，已逐处标注，测试完成后以实测数据"
...
def main():
    ...
    doc = Document()
    setup_page(doc)
    doc.core_properties.title = "智检桥安-命题解决方案（对标版）v5"
    ...
    perf_md5 = hashlib.md5(open(ppt_img("模型性能基线与提升_v2.png"), "rb").read()).hexdigest()
    ...
    print("图5-1新图嵌入:", "是" if perf_md5 in media_md5.values() else "否")
    print("v5 指标核查: 0.351 x%d, 0.158 x%d, 0.336 x%d, 0.085 x%d, P=0.414 x%d"
          % (full_text.count("0.351"), full_text.count("0.158"),
             full_text.count("0.336"), full_text.count("0.085"), full_text.count("0.414")))
    print("v5 新增内容: 获奖 ->", "挑战杯" in full_text and "龙图杯" in full_text,
          "; 闪面攻关 ->", "闪面" in full_text, "; git 60+ ->", "60+" in full_text,
          "; 调研占位 ->", "【待补充】次" in full_text)
