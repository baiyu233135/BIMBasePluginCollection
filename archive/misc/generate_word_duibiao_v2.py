# -*- coding: utf-8 -*-
"""
生成《智检桥安-命题解决方案（对标版）v2.docx》—— 对标沈阳建筑大学模板格式。

项目：智检桥安——基于人工智能与计算机视觉的桥梁表观损伤智能检测系统
命题：基于人工智能与计算机视觉的桥梁表观损伤智能检测系统开发（命题企业：广联达）
输出：docs/国创赛ppt/智检桥安-命题解决方案（对标版）v2.docx

v2 在 v1 基础上补充 A-G 七项缺口：
  A 目录后新增"命题响应对照与提交物清单"（表 0-1 十二项答题要求响应对照、表 0-2 九项提交物清单）；
  B 新增附录五（需求调研一手资料包：问卷/访谈提纲/纪要表模板）；
  C 第九章新增 9.5 专业课程与知识应用映射表（表 9-3）；
  D 第六章新增 6.1 实施计划与进度安排（表 6-1），原 6.1/6.2/6.3 顺延为 6.2/6.3/6.4，表 6-1→表 6-2；
  E 第二章开头插入图 2-1 检测业务流程前后对比；
  F 第九章新增 9.6 价值引领与社会责任；
  G 第九章 9.2 后新增成员实质性贡献说明表（表 9-2）。
...
OUT_PATH = os.path.join(PPT_DIR, "智检桥安-命题解决方案（对标版）v2.docx")
...
def setup_page(doc):
    ...
    hr = header_p.add_run("智检桥安——基于人工智能与计算机视觉的桥梁表观损伤智能检测系统")
    set_font(hr, east="宋体", size=9, color=(0x7F, 0x7F, 0x7F))
...
def build_abstract(doc):
    add_h1(doc, "项目摘要")
    add_body(doc, "截至2025年底，全国公路桥梁已超过100万座并大量进入运维期，表观病害的定期检测与经常性检查"
                  "任务日益繁重。传统检测模式中，图像采集、人工判读、尺寸测量、空间定位与报告编制五个环节"
                  "相互割裂：登高与桥检车抵近作业风险高、判读依赖个人经验、尺寸现场量测效率低、病害位置难以"
                  "三维归档、报告反复誊录易出错，检测数据与养护决策脱节。")
    add_body(doc, "针对命题企业“基于人工智能与计算机视觉的桥梁表观损伤智能检测系统开发”的真实业务需求，"
                  "本团队提出并实现了“智检桥安”系统：以国产BIMBase三维图形引擎为底座，构建“图像采集→损伤"
                  "智能识别与分类→损伤几何尺寸量化→三维空间定位→检测报告自动生成”的五环数字化闭环。系统采用"
                  "YOLOv8n专用模型与免模型五路CV掩码融合的双模式检测路线，基线模型在CPU环境下完成训练，"
                  "mAP50约0.336、mAP50-95约0.085，指标如实呈现并给出明确提升路线；集成Qwen-VL多模态大模型"
                  "完成病害成因诊断；以解析式2D→3D面投影将病害精确定位到BIM构件表面；基于python-docx一键生成"
                  "规范Word检测报告。")
    add_body(doc, "系统已形成“BIMBase桌面检测插件＋数字孪生网页平台”双形态成果：插件在常泰长江大桥真实工程"
                  "BIM模型（244.9MB，16类参数化构件）上完成全流程贯通验证；数字孪生平台为662KB单文件离线网页，"
                  "基于three.js实现大桥真实尺度程序化三维重建，支持构件显隐、病害JSON导入与三维贴片可视化，"
                  "浏览器免安装直接访问并可部署至公网。")
    add_body(doc, "本项目以低成本、轻量化、可复核的技术路线服务桥梁检测单位与管养单位，助力交通强国战略下"
                  "基础设施运维的数字化转型；方法可迁移至隧道、房屋等基础设施检测场景，具有良好的推广价值与"
                  "社会效益。")
...
def build_ch2(doc):
    add_h1(doc, "二、应用场景")
    add_body(doc, "本系统面向桥梁检测业务的四类典型场景提供数字化支撑，各场景的应用方式与所需补充的演示"
                  "截图如下。")
    add_body(doc, "传统检测模式与本方案的流程对比如图 2-1 所示：传统模式下，采集、判读、测量、定位、报告"
                  "五个环节由不同人员分步完成，数据在纸面与表格间反复誊录，环节割裂、数据断链；本方案以数字化"
                  "五环贯通同一检测任务，一人一键即可完成流转，数据自动衔接、全程可追溯。正是这一转变，使以下"
                  "四类应用场景得以高效落地。")
    add_fig(doc, ppt_img("对比图_检测流程前后对比.png"), "图 2-1  检测业务流程前后对比", width_cm=15.0)

    add_h2(doc, "2.1  定期检测辅助")
    add_body(doc, "为依法开展的定期检测提供影像智能判读结果与检测报告初稿：检测单位完成外业影像采集后，"
                  "由系统批量完成病害识别、尺寸量化与三维定位，内业人员聚焦复核与签认，大幅压缩内业整理时间。")
    add_clue(doc, "【截图线索】定期检测场景：插件批量导入某跨巡检照片并输出病害清单的界面截图")

    add_h2(doc, "2.2  经常性检查")
    add_body(doc, "面向高频次、低成本的日常巡查影像进行批量处理：管养单位巡查人员使用手机或便携相机拍摄的"
                  "照片即可导入系统，及时发现新增病害及其发展趋势，支撑“早发现、早处置”的预防性养护。")
    add_clue(doc, "【截图线索】经常性检查场景：手机拍摄照片导入插件并完成识别的界面截图")

    add_h2(doc, "2.3  灾后应急排查")
    add_body(doc, "台风、洪水、地震等灾害发生后，对批量巡检影像进行快速初筛：系统在短时间内完成大量影像的"
                  "病害识别与分布统计，辅助应急人员确定重点复查部位，为抢通保通决策提供参考。")
    add_clue(doc, "【截图线索】灾后应急场景：批量影像初筛结果统计与病害分布的界面截图")

    add_h2(doc, "2.4  无人机航线化巡检")
    add_body(doc, "配合无人机航线规划采集的标准化影像，实现照片批量自动分析与病害自动归档：航线化采集保证"
                  "拍摄角度与覆盖范围的一致性，与系统的面投影定位能力天然匹配，形成“航线采集—自动分析—"
                  "三维归档”的标准化作业流。")
    add_clue(doc, "【截图线索】无人机巡检场景：无人机航拍照片与病害三维落位对照的截图")

    add_h2(doc, "2.5  服务对象与系统定位边界")
    add_body(doc, "本系统的服务对象包括：（1）桥梁检测单位：提升影像判读、尺寸量化与报告编制效率，降低外业"
                  "作业风险；（2）桥梁管养单位：建立可视化的病害数字化档案，支撑养护决策与多期对比；（3）设计"
                  "施工单位：获取带空间定位的病害数据，为加固设计与维修施工提供数据支撑。")
...
def build_ch3(doc):
    ...
    add_body(doc, "系统将检测业务重构为五环闭环流程，如图 3-2 所示：（1）图像采集：以无人机巡检照片为主要"
                  ...
                  "几何尺寸量化：基于面投影标定的像素—物理尺度换算；（4）三维空间定位：解析式2D→3D面投影，"
...
def build_ch4(doc):
    add_h1(doc, "四、操作说明")
    add_body(doc, "本章按用户实际操作顺序说明系统使用方法。系统主界面如图 4-1 所示：左侧为操作面板，按"
                  "“导入照片—构件信息—尺寸标定—病害记录—生成报告”分区组织；右侧为照片浏览与已录入病害"
                  "列表。以下分九步说明完整操作流程。")
    ...
    steps = [
        ...
        ("4.5  第五步：尺寸量化",
         "系统基于第二步的标定比例，对检测框内的损伤自动完成掩膜细化与骨架提取，输出损伤长度、最大宽度与"
         "面积三项几何量，并随病害记录一并保存。",
...
def build_ch5(doc):
    ...
    add_body(doc, "本章阐述系统五环流程背后的四项核心技术：损伤智能识别（双模式）、损伤几何尺寸量化、解析式"
                  "面投影三维定位，以及数字孪生平台技术。全部关键算法均为团队自主实现，核心代码节选见附录三。")
    ...
    add_body(doc, "（1）尺度换算：损伤几何尺寸量化的核心是像素—物理尺度换算。系统提供两条标定路径：一是"
                  "参照物标定，用户在照片上框选已知尺寸的参照物，系统按参照物长边计算像素比例（mm/px）；二是"
                  "面投影标定，利用2D→3D面投影的几何对应关系获得成像面的物理尺度。两条路径共用同一套换算"
                  "逻辑，无需现场布设额外标定物；")
    add_body(doc, "（2）计算流程：尺寸量化按“检测框→掩膜细化→骨架提取→量化输出”四步处理，如图 5-2 所示。"
                  "检测框定位损伤感兴趣区域；掩膜细化获得贴合真实轮廓的损伤区域；骨架提取获得裂缝中轴线；最终"
                  "损伤长度取骨架长度乘以像素物理尺寸，最大宽度沿骨架法向逐点扫描取最大值，面积取掩膜像素数"
                  "乘以像素物理尺寸的平方；")
    add_fig(doc, ppt_img("思维导图_尺寸计算流程.png"), "图 5-2  损伤尺寸计算流程", width_cm=15.0)
    add_body(doc, "（3）精度目标：量化精度采用与人工测量对比的方式验证，选取覆盖各损伤类型的典型样本，由检测"
...
def build_ch7(doc):
    ...
    add_tbl(doc, "表 7-1  软件环境",
            ...
            [["三维平台", "BIMBase（国产三维图形引擎，支持pyp3d）", "插件运行底座、模型承载"],
             ...
             ["计算机视觉", "OpenCV", "五路CV掩码融合、掩膜细化与骨架提取"],
             ["界面框架", "PyQt5", "插件交互界面"],
...
def build_ch9(doc):
    ...
    add_body(doc, "【待补充】企业调研与试用记录：团队与命题企业及检测单位的调研纪要、试用反馈记录待整理后"
                  ...
                  "与纪要扫描件将按附录五说明归档补入。")
    add_body(doc, "通过本项目的完整工程实践，团队成员实现了从课程知识到工程能力的多维成长：（1）全栈开发"
                  "能力：从Python插件、桌面界面到three.js网页三维平台，独立完成跨端全栈实现；（2）AI工程能力："
                  "完成数据集建设、模型训练调优、多模态大模型工程化接入的完整闭环，并学会以如实口径呈现与"
                  "评价指标；（3）工程实践能力：在真实重大工程BIM模型上完成系统集成与验证，理解了检测业务流程"
                  "与工程合规要求；（4）职业认知：通过企业命题对接与需求分析，建立了“技术服务产业”的工程价值观，"
                  "明确了个人专业发展与国家基础设施数字化需求的结合点。")
    ...
    add_body(doc, "桥梁安全关乎公共安全，桥梁运维是交通强国建设的基础性工作。团队以青年学子的专业所长投身"
                  "基础设施智慧运维的真实问题，在工程实践中锤炼品格与本领，具体体现在三个方面：")
    add_body(doc, "（1）家国情怀：主动选用国产 BIMBase 三维图形引擎开展自主可控探索，把个人技术成长融入国家"
                  "关键软件自主化与交通强国战略的需要；")
    add_body(doc, "（2）工匠精神：坚持指标如实呈现、不作夸大——基线模型 mAP50 约 0.336 按实测写明，预期"
                  "指标逐处标注口径，测试数据与原始记录全程留痕、可复核、可查验；")
    add_body(doc, "（3）社会责任：坚持轻量化、低成本的技术路线，让基层管养单位“用得起、用得上”，以普惠的"
                  "数字化工具服务公共出行安全。")
...
def main():
    ...
    doc = Document()
    setup_page(doc)
    doc.core_properties.title = "智检桥安-命题解决方案（对标版）v2"
