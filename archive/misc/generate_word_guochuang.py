# -*- coding: utf-8 -*-
"""
生成国创赛产业赛道企业命题组《解决方案/命题对策书》Word 文档。

项目：智检桥安——基于人工智能与计算机视觉的桥梁表观损伤智能检测系统
命题：基于人工智能与计算机视觉的桥梁表观损伤智能检测系统开发
输出：docs/国创赛ppt/智检桥安_命题对策书_v1.docx
...
OUT_PATH = os.path.join(ROOT, "docs", "国创赛ppt", "智检桥安_命题对策书_v1.docx")
...
def build_ch2(doc):
    ...
    add_body(doc, "（2）数字孪生在线平台：网页端桥梁数字孪生平台，浏览器直接访问、免安装，以三维可视化方式"
                  "承载桥梁模型与病害档案，支持损伤分布查看、检测报告在线浏览、多期检测数据对比与劣化趋势追踪。")
...
def build_ch6(doc):
    ...
    add_body(doc, "团队已搭建网页端桥梁数字孪生平台，浏览器直接访问、免安装部署。平台以三维可视化方式承载"
                  "桥梁模型与病害档案，支持损伤分布查看、检测报告在线浏览、多期检测数据对比与劣化趋势追踪，"
                  "为管养单位提供持续性的数字化档案服务。")
...
def build_ch9(doc):
    ...
    add_body(doc, "（4）识别—量化—定位—报告全链路闭环与双形态交付：五环流程在桌面插件内一键贯通，同时以"
                  "数字孪生在线平台提供浏览器端的持续档案服务，实现“检测工具＋数据平台”双形态交付。")
...
def build_ch10(doc):
    ...
    add_table(doc, "表10-1  成果提交物清单对照表",
              ...
              [["1", "命题对策书/项目技术报告", "本文档", "全文", "已完成"],
               ...
               ["4", "可演示的数字化成果或交互原型", "BIMBase桌面检测插件、数字孪生在线平台",
                "第六章", "已完成"],
...
def main():
    doc = Document()
    setup_page(doc)
    doc.core_properties.title = "智检桥安 · 命题对策书"
