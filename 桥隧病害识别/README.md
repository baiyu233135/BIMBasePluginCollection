...
本模块**不单独注册 `.pyplugin`**，由 CADBoard 面板按钮转发调用：

```
BIMBase 面板「速构智维」→「桥梁病害识别」
  → CADBoard/bridge_disease_launcher.py（仅转发，sys.path 加入本目录）
  → 桥隧病害识别/bridge_disease_launcher.py::run_disease_recognition()
...
- **BIM 集成**：pyp3d（Component/Attr/Cube/Combine/create_material/UnifiedFunction），`bimbase_sync.py` 由 CADBoard 提供、动态路径导入
...
更详细的字段说明与注意事项见 `桥隧病害识别_使用说明.md`。

## 测试

| 文件 | 怎么跑 | 测什么 |
|---|---|---|
| `test_pier_projection_mapping.py` | `python 桥隧病害识别/test_pier_projection_mapping.py` | 投影映射纯数学验证（8 组断言，不依赖 pyp3d） |
| `test_t_beam_workflow.py` | `python 桥隧病害识别/test_t_beam_workflow.py` | T 梁全流程离线验证（需 cv2/python-docx） |
...
- 本模块从 CADBoard 拆分出来独立管理；`CADBoard/bridge_disease_launcher.py` 保留为转发入口，面板按钮路径不变。
- `bimbase_sync.py`（pyp3d 组件类与放置策略）仍由 CADBoard 提供，本模块通过动态路径导入复用。
