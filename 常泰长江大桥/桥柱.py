...
class 桥柱(Component):
    ...
    def replace(self):
        ...
        # 并列装配用 Combine（不做布尔并），仅真正的拱形开洞保留布尔减
        D = Combine(trans(ZD1/2-DW/2,ZW/2-DL/2,0)*DBB,trans(ZD2/2-DW/2,ZW/2-DL/2,0)*DBB)
        ...
        ZT = Combine(Loft(D1,Z1),Loft(Z1,S1),Loft(S1,S2),Loft(S2,H1))

        R1 = SZDL/13*6/2
        # 修正：原代码此行用 ZSJ/1892、下文统一用 ZSJ/1890，比例不一致会造成拱顶错缝，统一为 1890
        ...
        LTDL = Combine(trans(ZL/2-ZL/638*408/2,A/2-DLW/2,DH+XZJ-DLH/2)*LTDL1, trans(ZL/2-ZL/638*408/2,ZW-A/2-DLW/2,DH+XZJ-DLH/2)*LTDL1)
        B = ZL/2-ZL/638*408/2
        LTDW1 = scale(DLW,ZW/424*164,DLH)*Cube()
        LTDW = Combine(trans(B/2-DLW/2,A,DH+XZJ-DLH/2)*LTDW1, trans(ZL-B/2-DLW/2,A,DH+XZJ-DLH/2)*LTDW1)
        DS1 = scale(DSL,DLW,DSH)*Cube()
        DS = Combine(trans(ZL/2-ZL/638*408/2,A/2-DLW/2,DH+XZJ+DLH/2)*DS1, trans(ZL/2-ZL/638*408/2,ZW-A/2-DLW/2,DH+XZJ+DLH/2)*DS1, trans(ZL/2+ZL/638*408/2-DSL,A/2-DLW/2,DH+XZJ+DLH/2)*DS1, trans(ZL/2+ZL/638*408/2-DSL,ZW-A/2-DLW/2,DH+XZJ+DLH/2)*DS1)
        ALL_DLDS = Combine(LTDL, LTDW, DS)
        self['桥柱'] =  Combine(ZT, D, ALL_DLDS) - QP - QPW
