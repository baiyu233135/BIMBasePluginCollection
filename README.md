...
├── 常_常泰长江大桥/        # 常泰长江大桥参数化组件库（16 个真实工程尺度构件，3 大分类）
├── P3D文件/               # BIMBase 工程文件（.p3d；超 100MB 的不入库，本地保留）
...
| `常_常泰长江大桥.pyplugin` | 常泰长江大桥 | 1.3.0 | `常_常泰长江大桥/` | 16 个真实工程尺度构件，3 大分类 + 铁路封闭网 |
...
- [常_常泰长江大桥/README.md](常_常泰长江大桥/README.md) —— 常泰长江大桥参数化组件库
- [数据参考源/README.md](数据参考源/README.md) —— 原始资料与数据索引
- [P3D文件/README.md](P3D文件/README.md) —— BIMBase 工程文件与 100MB 限制说明
- [docs/README.md](docs/README.md) —— 项目级文档索引
- [archive/README.md](archive/README.md) —— 归档脚本索引

## ⚠️ 说明

- `桥梁组件/`、`AI建模/` 为早期成果，**不在本项目范围内**，仅作参考，相关文档不保证时效。
- `archive/` 存放开发期临时调试脚本；`数据参考源/` 为原始图纸与巡检资料。
- **大文件不入库**：超 GitHub 100MB 限制的文件（`P3D文件/常泰长江大桥.p3d` 244.9MB、`docs/挑战杯项目官方文件及参考/ppt制作与参考/` 下两个 pptx 235MB/111MB、`数据参考源/桥梁数据源/6.docx` 108MB）已从 git 历史清除并加入 .gitignore，仅本地保留；新增大文件请遵循同一做法。
